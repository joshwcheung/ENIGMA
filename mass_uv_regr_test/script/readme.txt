...
-----------------------------------------------
0. THIS IS THE BETA VERSION. You're strongly encouraged not to blindly belive the results, but to manually run several models on at least one ROI and compare the results. Especially the complex models which use filters and new regressors. After running the script you should go to the logs directory and check every log for the word "Error" and try to fix that.
...
2.2.3 Section 2. Main configuration section
	...
	CONFIG_PATH is the path to Google Sheets documents which contains the links to 2 other documents as well as Study ID, type (csv/raw) and Traits to be examined. YOU MUST CREATE YOUR OWN GOOGLE DOCS FILES FOR AnalysisList_Path and DemographicsList_Path fields in that document.
...
YOU MAY try to use MainFactor as factor(var1):factor(var2), BUT YOU MUST NOT USE it as Var1:factor(Var2) or factor(Var2):Var1, where Var1 is the continuos variable and Var2 has discrete levels.
ALSO, MainFactor HAS TO BE EITHER CONTINUOUS, OR HAVE ONLY TWO LEVELS in Covariates table. IF IT HAS MORE THAN TWO LEVELS, YOU MUST APPLY FILTERS TO LEAVE ONLY TWO LEVELS OF FACTOR FOR LINEAR MODEL.
...
JUST Q-SUB it!
qsub -t 1-#N# -q medium.q mass_uv_regr_IGC_csv.sh, where #N# is the Number of Nodes (Nnodes variable in your script)
or 
